...
i0 = float(sim_conf['initialCurrentMilliAmp'])
lifetime = sim_conf['beamLifeTimeSec']
noise = float(sim_conf['noisePercent']) / 100
...
for t in range(0, lifetime, 1):
    ...
    sock.sendall(f'name:VEPP/CURRENTS/FZ|method:set|value:{i}\n'.encode())

    for cam in svc_conf['cams']:
        cam_upper = cam.upper()
        ...
        if svc_conf['cams'][cam]['axes']['x']['weight'] != 0:
        ...
        if svc_conf['cams'][cam]['axes']['z']['weight'] != 0:
