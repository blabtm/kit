...
func Fork(w http.ResponseWriter, r *http.Request) {
	...
	fork, err := service.Fork(spec, []byte(req.Diff), req.Message)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
	...
	res := ForkResponse{IID: fork.IID.Short()}
...
func GetUI(w http.ResponseWriter, r *http.Request) {
	...
	schema, layout, err := service.GetUI(spec)
	if err != nil {
		log.Printf("error: %v\n", err)
		w.WriteHeader(http.StatusBadRequest)
		w.Write([]byte(fmt.Errorf("load: %v", err).Error()))
	...
	res := GetUIResponse{Schema: schema, Layout: layout}
	raw, err := json.Marshal(res)
	if err != nil {
		log.Printf("error: %v\n", err)
		w.WriteHeader(http.StatusInternalServerError)
